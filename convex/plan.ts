...
export const getAllUsersForAPlan = query({
  ...
  handler: async (ctx, args) => {
    ...
    const result = (await Promise.all(
      sharedAccessUserIds.map((userId) =>
        ctx.db
          .query("users")
    ...
    var final = result.map((r) => ({
      ...r,
      IsCurrentUser: r.userId == identity.subject,
    }));
    return final;
  },
});
    ...
    if (!args.isPublic) {
      ...
      const access = await ctx.db
        ...
        .withIndex("by_planId_userId", (q) =>
          q.eq("planId", args.id).eq("userId", subject)
        )
        .first();
      ...
      if (!admin.isPlanAdmin && !access) {
        throw new ConvexError(
          "The Plan you are trying to access either does not belong to you or does not exist."
        );
      }
...
export const prepareBatch1 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const modelName = completion as unknown as Pick<
        Doc<"plan">,
